  ...
  <div class="main">
    <div style="padding:20px">
      ...
      <el-form ref="form" label-width="120px" :inline="true">
        <el-form-item>
          <el-button type="primary" @click="handleCheck('add')">新增内容</el-button>
        </el-form-item>
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column align="center" label="内容" width="500" prop="content"></el-table-column>
        <el-table-column align="center" prop label="操作" width="220" fixed="right">
          <template slot-scope="scope">
            <el-button
              size="small" type="text"
              style="color:#E6A23C"
              @click="handleCheck('edit',scope.row)"
            >编辑</el-button>
            <el-button
              style="color:#F56C6C"
             size="small" type="text"
    ...
    <el-dialog
      ...
      <el-form ref="form" label-width="120px" :inline="true">
        ...
        <el-form-item label="内容">
          <el-input v-model="contentDetail" type="textarea"
...
import { dataStatistics, userIndex,keyWordsReplyList,keyWordsList, addKeyWordsReply,updateKeyWordsReply,deleteKeyWordsReply } from "@/api/robot";
...
export default {
  ...
  methods: {
    ...
    onSearch(e) {
      console.log(e)
      this.loading=true
      keyWordsList({keyword:e}).then(res => {
        this.loading=false
    ...
    handleCheckDel(e){
      this.$confirm('此操作将永久删除此数据, 是否继续?', '提示', {
          ...
          deleteKeyWordsReply({id:e.id}).then(res=>{
            this.getList()
          })
        }).catch(() => {
          this.$message({
    ...
    getList() {
      ...
      keyWordsReplyList(this.obj).then(res => {
        setTimeout(() => {
          loading.close();
        }, 300);
        // this.wechartList = res.data.data.robotAlias;
    ...
    checkCommunity(id) {
      if(this.contentId==''){
        addKeyWordsReply({ title: this.contentTitle,content:this.contentDetail,keyword_id:this.keywordId }).then(res => {
          this.$message(res.data.msg || "数据加载失败,请检查网络");
          this.getList()
          this.ReasonShow=false
        });
      }else{
        updateKeyWordsReply({ id: this.contentId,title: this.contentTitle,content:this.contentDetail,keyword_id:this.keywordId }).then(res => {
          this.$message(res.data.msg || "数据加载失败,请检查网络");
          this.getList()
          this.ReasonShow=false
        });

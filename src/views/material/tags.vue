        ...
        <el-form ref="form"
          ...
          <el-form-item>
            <el-button type="primary"
                       @click="handleCheck('add')">新增标签</el-button>
          </el-form-item>
...
import { dataStatistics, userIndex, getGroupTagList, addTag, updateGroupTag, delGroupTag } from "@/api/robot";
  ...
  methods: {
    ...
    handleCheckDel (e) {
      this.$confirm('此操作将永久删除此数据, 是否继续?', '提示', {
        ...
        delGroupTag({ tag_id: e.id }).then(res => {
          this.getList()
        })
      }).catch(() => {
        this.$message({
    ...
    getList () {
      ...
      getGroupTagList(this.obj).then(res => {
        setTimeout(() => {
          loading.close();
        }, 300);
    ...
    checkCommunity (id) {
      if (this.keywordId == '') {
        addTag({ tag_name: this.keywordValue }).then(res => {
          this.$message(res.data.msg || "数据加载失败,请检查网络");
          this.getList()
          this.ReasonShow = false
        });
      } else {
        updateGroupTag({ tag_id: this.keywordId, tag_name: this.keywordValue }).then(res => {
          this.$message(res.data.msg || "数据加载失败,请检查网络");
          this.getList()
          this.ReasonShow = false
        });
